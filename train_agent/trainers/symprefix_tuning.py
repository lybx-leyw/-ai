...
def prefix_train(
    ...
    if last_index<0:
        ...
        # 应用prefix层
        apply_prefix(model=model,scale=0.02)
        print("应用prefix成功")
    else:
        apply_prefix(model=model,scale=0.02)
        try:
            try:
                model = torch.load(f"out\\TinySeek_Prefix{last_index}_1.pkl").to(config['device'])
            except:
                model.load_state_dict(torch.load(f"out\\TinySeek_Prefix{last_index}_1.pkl"))
            print(f"成功加载模型参数: out\\TinySeek_Prefix{last_index}_1.pkl")
        except FileNotFoundError:
            print(f"未找到模型参数: out\\TinySeek_Prefix{last_index}_1.pkl")
    for param in model.parameters():
        param.requires_grad = False
    for name, module in model.named_modules():
        if hasattr(module, 'prefix'):
            module.prefix.prefix_1.requires_grad = True
            module.prefix.prefix_2.requires_grad = True
            print(f"Unfrozen prefix parameters in {name}")
    ...
    for input_number in range(n_of_samplings):
        ...
        with open(json_data_path, "r", encoding='utf-8') as data_file:
            ...
            all_ids = []
    torch.save(model.state_dict(),f"out\\TinySeek_Prefix_final.pkl")

def prefix_train_epoch(config,data,batch_size,max_epochs,
    ...
    dataset = PrefixDataset(data,pad_idx=config['pad_idx'],mask_start_id=9,mask_end_id=10)
    ...
    for epoch in range(max_epochs):
            ...
            if scaler is not None:
                ...
                if (batch_idx + 1) % accumulation_steps == 0:
                    ...
                    if scheduler is True:
                        ...
                        for param_group in optimizer.param_groups:
                            param_group['lr'] = max(5e-4*warmup_factor,1e-7)
            else:
                outputs_1,outputs_2,Lexp = model(batch_src)
                cross_loss_1 = F.cross_entropy(
                    outputs_1.reshape(-1,config['vocab_size']),
        ...
        if number//n_of_samples % save_frq == 0:
            torch.save(model.state_dict(),f"out\\TinySeek_Prefix{number}_{epoch+1}.pkl")