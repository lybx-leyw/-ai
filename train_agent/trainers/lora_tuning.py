...
def lora_train(
        ...
        seed=42,prefetch_factor=2,rank=16
        ): 
    random.seed(seed)
    np.random.seed(seed)
    ...
    if last_index<0:
        ...
        # 应用lora层
        apply_lora(model=model,rank=rank,scale=0.02)
        print("应用lora成功")
    else:
        apply_lora(model=model,rank=rank,scale=0.02)
        try:
            try:
                model = torch.load(f"out\\TinySeek_Lora{last_index}_1.pkl").to(config['device'])
            except:
                model.load_state_dict(torch.load(f"out\\TinySeek_Lora{last_index}_1.pkl"))
            print(f"成功加载模型参数: out\\TinySeek_Lora{last_index}_1.pkl")
        except FileNotFoundError:
            print(f"未找到模型参数: out\\TinySeek_Lora{last_index}_1.pkl")
    ...
    for input_number in range(n_of_samplings):
        ...
        with open(json_data_path, "r", encoding='utf-8') as data_file:
            ...
            all_ids = []
    torch.save(model.state_dict(),f"out\\TinySeek_Lora_final.pkl")

def lora_train_epoch(config,data,batch_size,max_epochs,
    ...
    for epoch in range(max_epochs):
        ...
        if number//n_of_samples % save_frq == 0:
            torch.save(model.state_dict(),f"out\\TinySeek_Lora{number}_{epoch+1}.pkl")